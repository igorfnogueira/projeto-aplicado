...
**TDS real da água de origem via 21 anos de relatórios da LADWP (`script_28_ladwp_tds_origem.py`, D-54).** A lacuna de dado mais crítica do projeto (D-30 — sem série de TDS da água de origem, só o proxy indireto do PDSI) foi parcialmente preenchida com dado primário real: 21 relatórios anuais de qualidade da água da LADWP (2004-2024), cada um publicando o TDS médio por fonte de abastecimento (LA Aqueduct, poços locais, e as três ETAs da MWD — Weymouth, Diemer, Jensen) e o percentual de mistura de cada ano. Construída uma série de TDS de origem ponderado e comparada ao TDS anual de efluente da LAGWRP (14 anos de sobreposição, 2011-2024): **r=0,912 (p=0,000005)** — mais forte que a correlação já obtida com o PDSI (D-37, LMG 67-79%). 7 dos 21 anos (cobrindo início/meio/fim da série e regimes climáticos distintos) foram conferidos manualmente célula a célula contra os PDFs originais, sem nenhuma discrepância. Limitações declaradas: o peso da MWD é uma média simples das suas 3 estações (o peso real de cada uma não é publicado); a série é anual, não mensal; cobre só o lado Los Angeles (LADWP) da área de serviço da LAGWRP, não o lado Glendale.

**Status:** a Etapa 3 ampliada está completa nos três métodos com maior valor causal (WRTDS, balanço de massa, cenários — D-39/D-40/D-41), todos confirmando e aprofundando o reenquadramento cíclico de D-14/D-37. GAM e os demais métodos da seção 3.f do plano original (espaço de estados, regressão quantílica, análise de intervenção, SVR, modelos fundacionais) ainda não foram implementados — decisão de escopo final ainda em aberto, ver `Artigo/DECISOES.md` D-43. Toda decisão, alternativa considerada e evidência de apoio está registrada em **[`Artigo/DECISOES.md`](Artigo/DECISOES.md)**, um log estilo ADR mantido continuamente junto com o código, junto com **[`GLOSSARIO.md`](GLOSSARIO.md)** (todo termo técnico explicado), **[`ESCOPO_E_LIMITACOES.md`](ESCOPO_E_LIMITACOES.md)** (fronteiras do estudo e fragilidades conhecidas) e **[`RESUMO_EXECUTIVO_DECISOES.md`](RESUMO_EXECUTIVO_DECISOES.md)** (mapa de 1 página das decisões que mudaram o rumo do projeto). Histórico completo de commits e código-fonte reproduzível em **https://github.com/igorfnogueira/projeto-aplicado**.
