...
**Status: aguardando a decisão do usuário sobre quais tratamentos adotar como padrão antes de prosseguir para a bateria de métodos ampliada (WRTDS, balanço de massa, cenários, espaço de estados, GAM, regressão quantílica, análise de intervenção, modelos fundacionais) — por instrução explícita do `prompt_tratamento_e_metodos.md`, isso não é decidido unilateralmente.**
