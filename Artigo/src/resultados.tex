...
\subsection{WRTDS: não há tendência de TDS depois de descontar a vazão}

A tendência bruta de $\log(\mathrm{TDS})$ (Theil-Sen) é $+0{,}582\%$/ano ($p=0{,}0056$, significativa). Após a flow-normalização (\texttt{script\_19\_wrtds.py}), a tendência cai para $-0{,}133\%$/ano (\textbf{$p=0{,}064$, não significativa}) -- quase toda a tendência aparente na série bruta desaparece ao descontar o efeito da vazão. A checagem de circularidade obrigatória (vazão derivada de Cloreto, independente do TDS, em vez da vazão padrão derivada do próprio TDS) confirma o resultado: tendência flow-normalized de $-0{,}158\%$/ano ($p=0{,}033$), correlação de $r=0{,}998$ entre as duas versões flow-normalized, diferença de apenas $0{,}024$ pontos percentuais/ano. A Figura \ref{fig:wrtds} mostra a série ajustada e flow-normalized; a Figura \ref{fig:wrtds-circ} mostra a checagem de circularidade.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{images/wrtds-flow-normalized.png}
    \caption{WRTDS: TDS ajustado e flow-normalized (efeito da vazão descontado).}
    \label{fig:wrtds}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{images/wrtds-circularidade.png}
    \caption{Checagem de circularidade: flow-normalização com vazão derivada de TDS vs.\ de Cloreto.}
    \label{fig:wrtds-circ}
\end{figure}

\textbf{Leitura honesta:} a tendência de alta identificada na Seção "Síntese comparativa" é, majoritariamente, um efeito da variação de vazão (ciclos de seca/conservação), não uma tendência independente de aumento de carga de sal -- consistente com D-37 e reforçando que o enquadramento correto é cíclico, não monotônico.

\subsection{Balanço de massa: diluição, não mais sal -- mas extrapolação linear é implausível}

\texttt{script\_20\_balanco\_massa.py} modela carga de sal (lb/dia, observada diretamente) e vazão (MGD, via Cloreto) separadamente. A carga de sal caiu $-3{,}20\%$/ano ($p<0{,}0001$) e a vazão caiu $-3{,}85\%$/ano ($p<0{,}0001$) no mesmo período -- \textbf{confirma o mecanismo de diluição}: entra cada vez menos água carregando uma quantidade de sal que também está caindo, não subindo. A validação histórica ($\mathrm{TDS} = \mathrm{carga} \div (\mathrm{vazao}_{\text{Cloreto}} \times 8{,}34)$ vs.\ observado) tem RMSE$=19{,}1$~mg/L, $r=0{,}973$, $R^2=0{,}941$ (Figura \ref{fig:balanco-validacao}). A checagem de tautologia (repetindo com a vazão do próprio TDS) dá $R^2=1{,}000000$ exato -- confirma que essa versão é uma identidade algébrica, não uma validação real.
...
\begin{figure}[H]
    ...
    \label{fig:balanco-decomp}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{images/balanco-massa-validacao.png}
    \caption{Validação histórica: TDS observado vs.\ TDS derivado (carga ÷ vazão de Cloreto).}
    \label{fig:balanco-validacao}
\end{figure}

\textbf{Achado negativo, registrado e não escondido:} a extrapolação linear (Theil-Sen) de carga e vazão para +10/+15/+20 anos cruza valores fisicamente implausíveis -- a vazão projetada cai para 14\% da capacidade nominal em +10 anos, 5\% em +15 anos, e fica \textbf{negativa} em +20 anos. O TDS derivado dessa extrapolação (993 $\to$ 1.724 $\to$ 1.557~mg/L, não-monotônico) não deve ser lido como previsão confiável -- é reportado para expor a fragilidade, motivando diretamente a próxima subseção.

\subsection{Projeção por cenários climáticos}

\texttt{script\_21\_cenarios.py} substitui a extrapolação linear implausível por uma projeção condicionada a cenários de seca, usando o PDSI histórico (AR(1): $\mu=-0{,}31$, $\phi=0{,}894$) e a relação $\mathrm{TDS} \sim \mathrm{PDSI}(\text{lag}=4\text{m})$ (coeficiente $=-20{,}4$, $R^2=0{,}445$). A Tabela \ref{tab:cenarios} e a Figura \ref{fig:cenarios-horizontes} mostram a faixa de TDS por cenário e horizonte (2.000 simulações de Monte Carlo por cenário); a Figura \ref{fig:cenarios-fan} mostra o cenário normal como \textit{fan chart} ao longo do tempo.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{images/cenarios-pdsi-fanchart.png}
    \caption{Projeção condicional do cenário normal (mediana + IC90\%) -- não é previsão pontual.}
    \label{fig:cenarios-fan}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:cenarios-horizontes}
\end{figure}

O cenário de agravamento climático converge para perto do cenário seco ao longo do horizonte, como esperado pelo desenho da rampa de média-alvo do PDSI. Todas as faixas são fisicamente plausíveis, ao contrário da extrapolação linear da subseção anterior. \textbf{Não se apresenta um valor único de TDS para 2046} -- a faixa de $\sim$590 a $\sim$850~mg/L, condicionada ao regime climático, é a resposta honesta que os dados permitem.

\subsection{Espaço de estados, GAM, regressão quantílica, análise de intervenção e modelo fundacional}

Completando os 9 métodos da seção 3.f do plano original, cinco métodos adicionais foram implementados (\texttt{script\_22} a \texttt{script\_26}), elevando a bateria de 27 para 34 métodos (35 após o 5º cenário climático de D-51).

\textbf{Espaço de estados (\texttt{local linear trend} + sazonal, escolhido por AIC):} a variância estimada do estado de tendência ficou praticamente zero ($2\times10^{-9}$, $p=1{,}000$) -- mesmo com liberdade para variar, o modelo não encontrou evidência de uma tendência genuinamente variável no tempo, resultado consistente com D-39 (WRTDS). Tendência média do estado: $+8{,}33$~mg/L/ano. \textit{Holdout}: RMSE$=50{,}16$, MASE$=0{,}52$ (não supera o \textit{naive}, $0{,}44$). Previsão $+20$a: $868{,}4$~mg/L, IC90 $[-36{,}8, 1773{,}6]$ -- o mais largo de toda a bateria.

\textbf{GAM:} minimizar o critério de ajuste (GCV) sem restrição no termo de tendência produziu extrapolação explosiva ($1.910$~mg/L em $+10$a, quase $3\times$ o último valor observado) -- patologia conhecida de \textit{P-splines}. Corrigido com um piso declarado no $\lambda$ da tendência ($\lambda\geq1{,}0$, ainda escolhido por GCV dentro dessa faixa), a previsão $+10$a passa a $576{,}5$~mg/L, com leve tendência de queda no longo prazo ($+20$a: $465{,}1$~mg/L, IC90 $[173{,}2, 756{,}9]$). \textit{Holdout}: RMSE$=61{,}87$, MASE$=0{,}68$.

\textbf{Regressão quantílica:} resultado na direção \textbf{oposta} à hipótese de trabalho -- a inclinação do percentil 10 é $+4{,}36$~mg/L/ano ($p=0{,}016$), a da mediana é $+3{,}49$ ($p=0{,}077$), e a do percentil 90 é $\mathbf{-3{,}05}$~mg/L/ano ($p=0{,}026$, significativa e negativa). Os picos de TDS não sobem mais rápido que a mediana -- pelo contrário, caem. Como os três quantis são ajustados de forma independente, a extrapolação \textbf{cruza} nos três horizontes (em $+20$a, o Q90 previsto, $709{,}1$~mg/L, fica abaixo do Q50, $794{,}8$~mg/L) -- limitação reportada, não corrigida por reordenação artificial dos valores.

\textbf{Análise de intervenção (SARIMAX com regressores de evento):} controlando pela ordem de conservação de 2015, o coeficiente da seca de 2012--2016 não atinge significância isoladamente ($+48{,}58$~mg/L, $p=0{,}206$); o coeficiente da ordem de conservação fica no limiar ($+92{,}82$~mg/L, $p=0{,}051$). A componente sazonal MA do modelo apresentou instabilidade de estimação (convergiu para o limite do espaço admissível, $\mathrm{ma.S.L12}=-1{,}0000$, erro-padrão $404$) -- registrado como limitação técnica, não escondido.

\textbf{Modelo fundacional \textit{zero-shot} (Chronos-Bolt-Small):} confirmando a expectativa declarada antes de rodar, o modelo pré-treinado não supera o \textit{baseline naive} (MASE$=0{,}59$ vs.\ $0{,}44$). Achado técnico adicional: a largura do intervalo (Q10--Q90, não IC90 -- limite nativo do modelo) \textbf{diminui} com o horizonte ($216{,}7$~mg/L em $+10$a $\to$ $137{,}8$ em $+20$a), o oposto do comportamento de todos os demais métodos -- característica conhecida de cabeças de previsão diretas/multi-horizonte, não um erro de implementação.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{images/espaco-estados-tds.png}
    \caption{Espaço de estados (UnobservedComponents) -- TDS observado, ajuste e previsão com IC90 nativo.}
    \label{fig:espaco-estados}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{images/gam-tds.png}
    \caption{GAM ($s$(tempo) + $s$(mês)) -- ajuste e previsão com o piso de suavização declarado (D-46).}
    \label{fig:gam}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{images/regressao-quantilica-tds.png}
    \caption{Regressão quantílica: mediana (Q50) vs.\ picos (Q90) vs.\ mínimos (Q10).}
    \label{fig:reg-quantilica}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{images/intervencao-arimax-tds.png}
    \caption{ARIMAX com regressores de evento (seca 2012-2016, ordem de conservação de 2015).}
    \label{fig:intervencao}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{images/modelos-fundacionais-tds.png}
    \caption{Modelo fundacional zero-shot (Chronos-Bolt-Small) -- sem treino nos dados do projeto.}
    \label{fig:fundacionais}
\end{figure}

\textbf{Síntese dos cinco métodos novos:} nenhum contradiz D-14/D-37/D-39/D-40 -- pelo contrário, reforçam de ângulos independentes que não há tendência residual clara além do efeito de vazão/regime, e que a extrapolação de longo prazo é estruturalmente frágil em múltiplas famílias de método (não só nas já conhecidas). Detalhes completos em \texttt{Artigo/DECISOES.md} (D-45 a D-50).

\subsection{Cenário climático fundamentado em projeção real (Cal-Adapt, RCP 8.5)}

A precipitação média projetada para 2035--2064 sob RCP~8.5, extraída no ponto da LAGWRP a partir de rasters reais do Cal-Adapt (ensemble de 32 modelos climáticos LOCA-\textit{downscaled}), cai apenas $2{,}3\%$ frente à média histórica de 1961--1990 ($1{,}244 \to 1{,}216$~mm/dia) -- mudança modesta, consistente com a incerteza conhecida das projeções de precipitação para a Califórnia (ao contrário de temperatura, cujas projeções são mais consensuais). Convertida num ``PDSI-alvo implícito'' via calibração empírica própria ($-0{,}69$, entre os cenários ``normal'' e ``seco''), a simulação de Monte Carlo produz uma faixa de $662$ a $670$~mg/L nos três horizontes (Figura \ref{fig:cenario-caladapt}) -- \textbf{entre} os cenários ``normal'' ($658$~mg/L em $+20$a) e ``seco''/``agravamento'' ($714$--$718$~mg/L), sem contradizer nem repetir os 4 cenários já existentes.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{images/cenario-caladapt-rcp85.png}
    \caption{Cenários históricos (\texttt{script\_21}) vs.\ cenário fundamentado em projeção climática real (Cal-Adapt, RCP 8.5) em +20 anos.}
    \label{fig:cenario-caladapt}
\end{figure}

\subsection{TDS de origem (LADWP) explica o TDS de efluente melhor que o PDSI}

A lacuna crítica identificada em D-30 (\texttt{Artigo/DECISOES.md}) -- ausência de uma série real de TDS da água de origem, hoje aproximada apenas pelo PDSI -- foi parcialmente preenchida com dados públicos primários: 21 relatórios anuais de qualidade da água da LADWP (2004--2026), cada um trazendo o TDS médio por fonte de abastecimento (\textit{Los Angeles Aqueduct}, poços locais, e as três estações de tratamento da MWD -- Weymouth, Diemer e Jensen -- que tratam a água importada do \textit{Colorado River Aqueduct}/\textit{State Water Project}) e o percentual de mistura de cada fonte naquele ano (\texttt{script\_28\_ladwp\_tds\_origem.py}). Para cada ano, um TDS de origem ponderado foi calculado como a média das fontes ponderada pelo percentual de mistura publicado. Os 21 anos foram conferidos manualmente célula a célula contra o PDF original (auditoria de 100\%, não amostragem), sem nenhuma discrepância -- exceto uma inconsistência já presente no relatório original da LADWP de 2015 (não introduzida pela extração), preservada como está na fonte.

Comparado ao TDS médio anual de efluente da LAGWRP (14 anos de sobreposição, 2011--2024), a correlação de Pearson é $r=0{,}912$ ($p=0{,}000005$) -- mais forte que a correlação já obtida com o PDSI via decomposição LMG (D-37, 67--79\%). O percentual de \textit{LA Aqueduct} no mix (água menos salina) correlaciona negativamente com o TDS de efluente, na direção esperada: $r=-0{,}750$ ($p=0{,}002$).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{images/ladwp-tds-origem-vs-efluente.png}
    \caption{TDS de origem ponderado (LADWP, 2004--2024) vs.\ TDS de efluente da LAGWRP: série anual (esquerda) e dispersão com reta de tendência (direita).}
    \label{fig:ladwp-origem}
\end{figure}

\textbf{Limitações declaradas:} (1) o peso usado para a MWD é uma média simples das três estações de tratamento, não o peso real de cada uma dentro do total importado (não publicado em nenhum dos 21 relatórios); (2) a correlação é anual, não mensal -- os relatórios não publicam série mensal, o que impede testar defasagem fina como foi feito com o PDSI; (3) o relatório cobre o abastecimento da cidade de Los Angeles (lado LADWP), não o lado Glendale da área de serviço da LAGWRP, cujo dado equivalente ainda não foi obtido. Detalhes completos em D-54.

\subsection{Reconciliação entre as famílias de método}

Os métodos de tendência/ML de \texttt{script\_01}--\texttt{script\_15} (finalistas $\sim$760-800~mg/L em +20a, Seção "Síntese final") não são contraditados pelos resultados desta seção -- respondem a uma pergunta diferente (desempenho preditivo de curto prazo e magnitude da tendência bruta, sem descontar vazão). WRTDS responde se há tendência \textit{sob} os ciclos (não, uma vez descontada a vazão); balanço de massa responde se o mecanismo é diluição ou mais sal (diluição); cenários respondem qual a faixa plausível de longo prazo condicionada ao clima (594-721~mg/L em +20a, ante $\sim$760-800~mg/L da extrapolação de tendência bruta) -- a diferença entre as duas faixas é esperada e coerente: a extrapolação de tendência bruta carrega implicitamente o efeito de vazão continuando a cair, enquanto os cenários limitam essa vazão a valores fisicamente plausíveis (D-42).

\subsection{Comparação com a literatura}

Três estudos dão contexto ao mecanismo de diluição confirmado neste trabalho (Seção "Balanço de massa": carga de sal caindo, vazão caindo mais rápido). Schwabe et al.\ \cite{schwabe2020unintended} encontraram, em 34 estações de tratamento do sul da Califórnia (2013--2017), que a conservação de água durante a seca reduziu o volume de esgoto e aumentou sua salinidade -- a mesma direção de efeito, no mesmo tipo de instalação e na mesma região, que o mecanismo aqui identificado na LAGWRP. Wolfand et al.\ \cite{wolfand2022dilution} modelaram a mesma bacia hidrográfica onde a LAGWRP descarrega (rio Los Angeles) e encontraram que a concentração de TDS aumenta com o reúso de água, mesmo quando a carga total de poluentes diminui -- reforçando, em nível de bacia e não só de estação individual, o mesmo mecanismo de concentração por redução de volume. O texto completo de nenhum dos dois foi acessível nesta sessão (ambos pagos); a citação se apoia em resumos institucionais/de sociedade profissional, que confirmam o desenho do estudo e a direção do efeito, mas não a magnitude -- por isso nenhum número desses dois estudos é reproduzido aqui.

O terceiro estudo, da Southern California Salinity Coalition \cite{scsc2018tds}, é o único dos três cujo texto completo foi efetivamente lido nesta sessão, e é o mais influente sobre o enquadramento final deste trabalho: usando decomposição LMG em 26 estações da mesma região, os autores atribuem ~88\% da variabilidade do TDS de influente ao TDS da própria água de abastecimento (que varia com ciclos de seca via a origem da água importada), contra ~12\% do consumo per capita -- ou seja, a conservação local (mecanismo de Schwabe et al.) é real, mas secundária. Esse achado motivou diretamente o teste do PDSI como variável explicativa (Seção "Tratamento de dados robusto e reenquadramento cíclico") e a decomposição LMG análoga em \texttt{script\_18}/\texttt{script\_19} deste trabalho. \textbf{Concordância na direção do efeito e no mecanismo proposto} entre os três trabalhos: conservação/reúso de água reduz o volume de esgoto/vazão do rio, elevando o TDS numa água mais concentrada -- mas o achado da SCSC, obtido com dados de 26 estações, é o que efetivamente explica \emph{por que} este trabalho encontrou uma série cíclica (ligada a ciclos de seca da água de origem) em vez de uma tendência monotônica simples.
