...
- **Data:** 2026-08-19 · **Status:** **Pendente** (registro do gap, decisão de escopo ainda não tomada explicitamente)
- **Contexto:** `plano_projeto_TDS.md` §3.f previa 9 métodos adicionais (WRTDS, balanço de massa, cenários, espaço de estados/DLM, **GAM**, regressão quantílica, análise de intervenção, SVR, modelos fundacionais). Dos 9, apenas WRTDS (D-39), balanço de massa (D-40) e cenários (D-41) foram implementados — os demais (GAM incluído) não. `pygam` já foi instalado no ambiente numa sessão anterior, mas o script correspondente nunca foi escrito.
- **Pergunta em aberto:** isso é uma decisão de escopo deliberada (ex. "WRTDS/balanço de massa/cenários já respondem à pergunta causal do trabalho com robustez suficiente; os métodos restantes seriam redundantes ou de baixo retorno para o tempo disponível") ou é uma pendência de tempo que ainda deveria ser executada antes da entrega?
- <!-- PENDENTE: decisão do usuário -- fechar o escopo da bateria aqui (e registrar o motivo: redundância com WRTDS/balanço/cenários, restrição de tempo, ou outro) ou continuar implementando GAM/DLM/regressão quantílica/análise de intervenção/SVR/modelos fundacionais antes da entrega? -->
- **Por que isso importa para a defesa:** uma banca pode perguntar diretamente por que a seção 3.f do plano não foi 100% executada. Ter a resposta registrada aqui evita ter que improvisar uma justificativa no momento da pergunta.
- **Evidência:** `plano_projeto_TDS.md` §3.f; comparação com os scripts existentes (`script_16` a `script_21`); `requirements.txt` (linha `pygam`, instalado mas não utilizado).
