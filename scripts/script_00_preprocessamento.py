...
def construir_datasets():
    tds = serie_mensal_canonica(carregar_csv(raw("TDS.csv")), PARAMETROS["TDS"])
    chl = serie_mensal_canonica(carregar_csv(raw("Chloride.csv")), PARAMETROS["Chloride"])
    amm = serie_mensal_canonica(carregar_csv(raw("Ammonia.csv")), PARAMETROS["Ammonia"])
    bod = serie_mensal_canonica(carregar_csv(raw("BOD.csv")), PARAMETROS["BOD"])
...
def main():
    ...
    dataset_a.to_csv(processed("dataset_canonico_bod_mdl2.csv"), sep=";", decimal=",", index=False)
    dataset_b.to_csv(processed("dataset_canonico_bod_zero.csv"), sep=";", decimal=",", index=False)
    dataset_c.to_csv(processed("dataset_canonico_bod_ros.csv"), sep=";", decimal=",", index=False)
