...
PRECIP_RAW_FILE = raw("pdsi_raw_climdiv_precip_estadual.txt")
...
RESULTADOS_CSV = RESULTADOS_COMPARACAO_CSV
RESULTADOS_JSON = RESULTADOS_COMPARACAO_JSON
CENARIOS_RESULTADOS_JSON = result("cenario_caladapt_resultados.json")
FIGURA_PATH = figure("cenario-caladapt-rcp85.png")
...
def main():
    ...
    pdsi = pd.read_csv(processed("pdsi_california_estadual.csv"), parse_dates=["Data"]).set_index("Data")["pdsi"]
    ...
    lag = pd.read_csv(result("pdsi_regimes_resultados.csv"))
    ...
    with iniciar_run("cenario_pdsi_rcp85_caladapt", "script_27_cenario_climatico_caladapt",
        ...
        comparacao = pd.read_csv(result("cenarios_resultados.csv"))
