...
**Real source-water TDS via 21 years of LADWP reports (`script_28_ladwp_tds_origem.py`, D-54).** The most critical data gap (D-30 — no source-water TDS series, only the indirect PDSI proxy) is now partially filled with real primary-source data: 21 annual LADWP water-quality reports (2004-2024) each publish average TDS by supply source (LA Aqueduct, local wells, and the MWD's three treatment plants — Weymouth, Diemer, Jensen) plus the yearly supply-mix percentage. A weighted source-TDS series was built from this and compared to LAGWRP's annual effluent TDS (14 overlapping years, 2011-2024): **r=0.912 (p=0.000005)** — stronger than the PDSI correlation (D-37, LMG 67-79%). 7 of the 21 years (spanning the start/middle/end of the series and varied climate regimes) were manually verified cell-by-cell against the source PDFs with zero discrepancies. Declared limitations: MWD weight is a simple average across its 3 plants (the real per-plant blend ratio isn't published); the series is annual, not monthly; it covers only the Los Angeles (LADWP) side of LAGWRP's service area, not Glendale's.

**Status:** the expanded Etapa 3 is complete for the three highest-causal-value methods (WRTDS, mass balance, scenarios — D-39/D-40/D-41), all confirming and deepening the D-14/D-37 cyclical reframing. GAM and the remaining §3.f methods from the original plan (state-space, quantile regression, intervention analysis, SVR, foundation models) have not been implemented yet — final scope decision still open, see `Artigo/DECISOES.md` D-43. Every decision, alternative considered, and piece of supporting evidence is tracked in **[`Artigo/DECISOES.md`](Artigo/DECISOES.md)**, an ADR-style log maintained continuously alongside the code, along with **[`GLOSSARIO.md`](GLOSSARIO.md)** (every technical term explained), **[`ESCOPO_E_LIMITACOES.md`](ESCOPO_E_LIMITACOES.md)** (explicit study boundaries and known fragilities), and **[`RESUMO_EXECUTIVO_DECISOES.md`](RESUMO_EXECUTIVO_DECISOES.md)** (1-page map of the decisions that changed the project's direction). Full commit history and reproducible source at **https://github.com/igorfnogueira/projeto-aplicado**.
