...
**Status:** the expanded method battery (WRTDS, mass balance, scenarios, GAM, etc., per `prompt_tratamento_e_metodos.md` Etapa 3) has not started — the cyclical/regime finding above changes what those methods should test (drought-cycle response, not a monotonic trend) and that reframing is still awaiting the user's decision, consistent with this project's rule of never fixing a methodological choice unilaterally. Every decision, alternative considered, and piece of supporting evidence is tracked in **[`Artigo/DECISOES.md`](Artigo/DECISOES.md)**, an ADR-style log maintained continuously alongside the code. Full commit history and reproducible source at **https://github.com/igorfnogueira/projeto-aplicado**.
