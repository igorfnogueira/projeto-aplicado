...
**Status:** Step 1 of the expanded Etapa 3 (this drought test) is done and confirms the reframing. The remaining expanded-battery methods (WRTDS, mass balance, scenarios, GAM, etc., per `prompt_tratamento_e_metodos.md`) have not started yet, and will be built around the drought/regime pattern rather than a monotonic trend. Every decision, alternative considered, and piece of supporting evidence is tracked in **[`Artigo/DECISOES.md`](Artigo/DECISOES.md)**, an ADR-style log maintained continuously alongside the code, along with **[`GLOSSARIO.md`](GLOSSARIO.md)** (every technical term explained) and **[`ESCOPO_E_LIMITACOES.md`](ESCOPO_E_LIMITACOES.md)** (explicit study boundaries and known fragilities). Full commit history and reproducible source at **https://github.com/igorfnogueira/projeto-aplicado**.
